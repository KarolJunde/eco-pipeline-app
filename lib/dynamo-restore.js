...
function DynamoRestore(options) {
    ...
    options.awsRegion = options.awsRegion || process.env.AWS_DEFAULT_REGION || 'eu-central-1';
...
DynamoRestore.prototype._checkTableReady = function(error, data) {
    ...
    if (data &&
        ...
        setTimeout(dynamodb.describeTable.bind(dynamodb, { TableName: data.Table.TableName }, this._checkTableReady.bind(this)), 2000);
    }
};
...
DynamoRestore.prototype._sendBatch = function() {
    ...
    this.requests.push(dynamo.batchWriteItem(params, (function(error, data) {
        ...
        if (this.tableready && this.requests.length < options.concurrency * 0.5) {
            this.readline.resume();
        }
